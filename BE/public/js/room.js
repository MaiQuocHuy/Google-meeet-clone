...
function handleVideoOffer(offer, sid, socketInfo) {
    ...
    connections[sid].onicecandidate = function (event) {
        if (event.candidate) {
            console.log('icecandidate fired offered');
    ...
    connections[sid].ontrack = function (event) {
        ...
        if (!document.getElementById(sid)) {
            console.log('track event fired offered')
            ...
            name.innerHTML = `${socketInfo.socketName}`;
            vidCont.id = sid;
            ...
            if (socketInfo.micSocket == 'on')
                muteIcon.style.visibility = 'hidden';
            else
                muteIcon.style.visibility = 'visible';

            if (socketInfo.videoSocket == 'on')
    ...
    connections[sid].onremovetrack = function (event) {
        if (document.getElementById(sid)) {
            ...
            console.log('removed a track offered');
        }
    };
    ...
    connections[sid].onnegotiationneeded = function () {
        ...
        connections[sid].createOffer()
            .then(function (offer) {
                console.log("onnegotiationneeded offered");
                return connections[sid].setLocalDescription(offer);
            })
            .then(function () {
                socket.emit('video-offer', connections[sid].localDescription, sid,roomid);
...
socket.on('join room', async (conc) => {
    ...
    if (conc) {
        await conc.forEach(socketInfo => {
            ...
            connections[sid].onicecandidate = function (event) {
                if (event.candidate) {
                    console.log('icecandidate fired join');
                    socket.emit('new icecandidate', event.candidate, sid);
                }
            };
            ...
            connections[sid].ontrack = function (event) {
                ...
                if (!document.getElementById(sid)) {
                    console.log('track event fired join')
                    let vidCont = document.createElement('div');
                    let newvideo = document.createElement('video');
                    let name = document.createElement('div');
                    let muteIcon = document.createElement('div');
            ...
            connections[sid].onremovetrack = function (event) {
                if (document.getElementById(sid)) {
                    console.log("onremovetrack join");
                    document.getElementById(sid).remove();
                }
            }
            ...
            connections[sid].onnegotiationneeded = function () {
                ...
                connections[sid].createOffer()
                    ...
                    .then(function () {
                        console.log("onnegotiationneeded join");
                        //sid of receiver
                        socket.emit('video-offer', connections[sid].localDescription, sid,socketInfo);
                    })
                    .catch(reportError);
            };
